...
void filterDictionary(info_t *info, tree_t *head, _Bool *isEnough, int length, int *counter);
...
//	CLOSE TO I/O  //
////////////////////
void filterDictionary(tree_t *h)
{
	list_t *listCursor;
	...
	if (h->character != EOS)
	{
		...
		else if (counter[characterIndex] > info->discoveredOccurrences[characterIndex] && info->isDefinitive[characterIndex])
		{
		 ...
		 else if (counter[characterIndex] >= info->discoveredOccurrences[characterIndex])
		 {
			 // segno che non ci sono problemi
			 isEnough[characterIndex] = 1;
		 }
		 else
		 {
			 // segno che ci sono problemi
			 isEnough[characterIndex] = 0;
		 }
		 if (h->child == NULL)
		 {
			 for (i = 0; i < length; i++)
	...
	for (listCursor = h->child; listCursor != NULL; listCursor = listCursor->nextSibiling)
		if (listCursor->node->valid)
			filterDictionary(info, listCursor->node, isEnough, length, counter);
	;
}
