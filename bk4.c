...
list_t *findChild(list_t *h, int c);

_Bool findWord(tree_t *head, char *word, int length);

tree_t *addNode(tree_t *h, int c);

list_t *addChildBetween(list_t *last, list_t *next, int c);

list_t *findOrAddChild(list_t *h, int c, list_t **found);
...
_Bool findWord(tree_t *head, char *word, int length)
{
	...
	for (i = 0; i < length; i++)
	{
		...
		rightChild = findChild(treeCursor->child, characterToIndex(word[i]));
...
tree_t *addWord(tree_t *head, char *word, int length)
{
	...
	for (i = 0; i < length; i++)
	{
		...
		treeCursor->child = findOrAddChild(treeCursor->child, characterToIndex(word[i]), &rightChild);
...
list_t *findOrAddChild(list_t *h, int c, list_t **found)
{
	...
	for (cursor = h; cursor != NULL && cursor->node->index < c; cursor = cursor->nextSibiling)
		previous = cursor;
	// se non lo trovo lo aggiungo
	if (cursor == NULL || cursor->node->index > c)
	{
...
void compareWords(info_t *info, char *word, char *solution, char *result, _Bool *isFree, int length, int *counter)
{
	short unsigned int i, j, found, definitive, characterIndex;
